...
## Corpus Check
- 70 files · ~346,234 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 773 nodes · 1189 edges · 35 communities detected
...
3. `ValidationPhase` - 30 edges
4. `PipelineTracer` - 29 edges
5. `PRAssemblyPhase` - 28 edges
6. `PipelineLogger` - 26 edges
7. `InputAnalysisPhase` - 25 edges
8. `MetricsCollector` - 24 edges
...
- `initializePipeline()` --calls--> `createConfiguredOrchestrator()`  [INFERRED]
  app\api\pipeline\route.ts → lib\pipeline\core\orchestrator.ts
...
Cohesion: 0.09
Nodes (9): createDevelopmentObservability(), createMinimalObservability(), createObservability(), createProductionObservability(), getDefaultObservability(), createResilience(), executeResilient(), ResilienceManager (+1 more)
...
Cohesion: 0.15
Nodes (9): buildDefensivePrompt(), buildEliminationPrompt(), buildRootCausePrompt(), buildTestGenerationPrompt(), normalizeAIResponse(), safeParseAI(), sanitizeHypothesis(), sanitizeRootCause() (+1 more)
...
Cohesion: 0.11
Nodes (9): createCacheManager(), createDevCacheManager(), createCacheKey(), handleStreamingRequest(), handleSynchronousRequest(), initializePipeline(), isValidGitHubUrl(), POST() (+1 more)

### Community 18 - "Community 18"
Cohesion: 0.16
Nodes (2): createConfiguredOrchestrator(), PipelineOrchestrator
...
- **Thin community `Community 8`** (29 nodes): `pr-assembly.ts`, `createPRAssemblyPhase()`, `PRAssemblyPhase`, `.calculateConfidence()`, `.createFileChanges()`, `.createTestFiles()`, `.executePhase()`, `.extractInput()`, `.formatCategory()`, `.formatErrorType()`, `.generateBranchName()`, `.generateBreakingChangesSection()`, `.generateChangesSummary()`, `.generateChecklist()`, `.generateDetailedChanges()`, `.generateLabels()`, `.generatePRDescription()`, `.generatePRTitle()`, `.generateQualityMetrics()`, `.generateRollbackInstructions()`, `.generateSummary()`, `.generateTestingInstructions()`, `.generateTestsSection()`, `.generateValidationSection()`, `.getResourceUsage()`, `.handleError()`, `.suggestReviewers()`, `.truncate()`, `.validate()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 11`** (26 nodes): `input-analysis.ts`, `createInputAnalysisPhase()`, `InputAnalysisPhase`, `.analyzeRepository()`, `.calculateClassificationConfidence()`, `.classifyIssue()`, `.detectFramework()`, `.detectLanguage()`, `.detectRepositoryLanguage()`, `.determineComplexity()`, `.determineIssueType()`, `.determineSeverity()`, `.executePhase()`, `.extractErrorContext()`, `.extractInput()`, `.extractTags()`, `.findRelatedFiles()`, `.generateClassificationReasoning()`, `.getAllFiles()`, `.getDirectorySize()`, `.getResourceUsage()`, `.identifyRelevantFiles()`, `.parseStackFrame()`, `.parseStackTrace()`, `.validate()`, `.validateOutput()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 13`** (25 nodes): `metrics.ts`, `MetricsCollector`, `.buildPhaseMetrics()`, `.buildResourceUsage()`, `.calculateCostMetrics()`, `.calculatePerformanceMetrics()`, `.calculateQualityMetrics()`, `.calculateStats()`, `.constructor()`, `.exportPrometheus()`, `.getCustomMetricStats()`, `.getMetrics()`, `.getPhaseMetrics()`, `.percentile()`, `.recordAICall()`, `.recordCacheHit()`, `.recordCacheMiss()`, `.recordCustomMetric()`, `.recordError()`, `.recordPhaseCacheHit()`, `.recordPhaseEnd()`, `.recordPhaseResourceUsage()`, `.recordPhaseRetry()`, `.recordPhaseStart()`, `.reset()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 14`** (25 nodes): `.generateHypotheses()`, `ai-reasoning.ts`, `AIReasoningPhase`, `.analyzeRootCause()`, `.buildAIContext()`, `.buildHypothesisPrompt()`, `.buildReasoningChain()`, `.calculateOverallConfidence()`, `.categorizeError()`, `.estimateComplexity()`, `.estimateSideEffects()`, `.executePhase()`, `.extractInput()`, `.generateFallbackHypotheses()`, `.generateFixStrategies()`, `.generateHypotheses()`, `.generateReasoningExplanation()`, `.getResourceUsage()`, `.handleError()`, `.identifyFilesToExamine()`, `.parseHypothesesFromResponse()`, `.rankHypotheses()`, `.validate()`, `.validateOutput()`, `createAIReasoningPhase()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 18`** (22 nodes): `createConfiguredOrchestrator()`, `createOrchestrator()`, `PipelineOrchestrator`, `.buildOutput()`, `.buildTimeline()`, `.calculateProgress()`, `.calculateQuality()`, `.cancel()`, `.emitProgress()`, `.execute()`, `.executePhase()`, `.getExecutionState()`, `.getMetrics()`, `.getPhaseTimeout()`, `.getStatus()`, `.handlePhaseError()`, `.listActiveExecutions()`, `.registerDefaultPhases()`, `.registerPhase()`, `.resume()`, `.validatePhaseTransition()`, `orchestrator.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 23`** (17 nodes): `createValidationGate()`, `createValidationRule()`, `validateTransition()`, `ValidationGate`, `.addRule()`, `.clearFailedRules()`, `.constructor()`, `.disableRule()`, `.enableRule()`, `.getAllRules()`, `.getApplicableRules()`, `.getFailedRules()`, `.getRule()`, `.initializeDefaultRules()`, `.removeRule()`, `.validate()`, `validation-gate.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 24`** (17 nodes): `ObservabilityManager`, `.constructor()`, `.endPhase()`, `.exportAll()`, `.getLogger()`, `.getMetrics()`, `.getMetricsCollector()`, `.getTrace()`, `.getTracer()`, `.mapLogLevel()`, `.recordAICall()`, `.recordCacheHit()`, `.recordCacheMiss()`, `.recordCheckpoint()`, `.recordError()`, `.reset()`, `.startPhase()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 26`** (13 nodes): `retry-manager.ts`, `createRetryManager()`, `RetryManager`, `.calculateBackoff()`, `.constructor()`, `.createRetryError()`, `.executeWithRetry()`, `.executeWithRetryResult()`, `.getStatistics()`, `.resetStatistics()`, `.shouldRetry()`, `.sleep()`, `withRetry()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 29`** (9 nodes): `getWatsonxClient()`, `WatsonxClient`, `.constructor()`, `.generate()`, `.generateBatch()`, `.generateStream()`, `.getIAMToken()`, `.healthCheck()`, `watsonx-client.ts`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
- **Thin community `Community 30`** (8 nodes): `helpers.ts`, `assertPhaseFailure()`, `assertPhaseSuccess()`, `createMockFileContent()`, `delay()`, `randomString()`, `validateRequiredFields()`, `wait()`
  Too small to be a meaningful cluster - may be noise or needs more connections extracted.
...
- **Why does `GraphTraversalPhase` connect `Community 2` to `Community 10`?**
  _High betweenness centrality (0.035) - this node is a cross-community bridge._
- **Why does `FixGenerationPhase` connect `Community 1` to `Community 10`?**
